...
class TestCacheable(unittest.TestCase):
    ...
    def test_generate_lru_key(self):
        ...
        for args, argkv, expect_str in cases:
            self.assertEqual(k3cacheable.Cacheable()._arg_str(args, argkv),
                             expect_str)
...
@k3cacheable.cache('concurrent_update_cache_data', capacity=100, timeout=60,
                 is_deepcopy=False, mutex_update=False)
def get_concurrent_update_cache_data():
    time.sleep(3)
    return {}

@k3cacheable.cache('mutex_update_cache_data', capacity=100, timeout=60,
